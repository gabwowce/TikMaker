...
import { hasKeyframePath, keyframesFor, poseAtFrame } from "../visualKeyframes";
function layer(
  keyframes: PositionedVisualEntry["keyframes"],
...
describe("keyframe tracks", () => {
  ...
  it("counts a path per track, not across the whole array", () => {
    expect(
      hasKeyframePath(
        layer([
          { id: "a", frame: 0, x: 1, y: 1 },
          { id: "b", frame: 9, scale: 2 },
        ]),
      ),
    ).toBe(false);
    expect(
