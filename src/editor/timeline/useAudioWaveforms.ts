...
export function sliceWaveform(
  ...
  const to = Math.max(
    from + 1,
    Math.ceil((peaks.length * (start + length)) / Math.max(1, total)),
  );
  ...
  const task = (async () => {
    try {
      ...
      const peaks = Array.from({ length: count }, (_, index) => {
        ...
        let peak = 0;
        for (let at = from; at < to; at += Math.max(1, Math.floor(stride / 80)))
          peak = Math.max(peak, Math.abs(samples[at] ?? 0));
        return peak;
      });
      const largest = Math.max(0.01, ...peaks);
      const result = {
        durationInFrames: Math.max(1, Math.round(buffer.duration * fps)),
        peaks: peaks.map((peak) => peak / largest),
      };
...
export function cachedAudioDuration(
...
export function useAudioWaveforms(
  sources: string[],
  fps: number,
): Map<string, AudioWaveform> {
  const key = [...new Set(sources.filter(Boolean))].sort().join("\n");
  const [, redraw] = useReducer((value) => value + 1, 0);
  useEffect(() => {
    let live = true;
    const unique = [...new Set(sources.filter(Boolean))];
    void Promise.all(unique.map((src) => decode(src, fps))).then(() => {
      if (live) redraw();
    });
    return () => {
      live = false;
  ...
  return new Map(
    [...new Set(sources.filter(Boolean))].flatMap((src) => {
