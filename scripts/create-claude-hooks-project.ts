...
const beats = [
  {
    role: "hook",
    purpose:
      "Challenge the false confidence created by passing isolated tests.",
    visualBrief:
      "Claude marks move across the frame while the gap between tests and reality lands.",
  ...
  },
  {
    role: "reveal",
    purpose:
      "Reveal that Claude can verify the behavior users actually experience.",
  ...
  },
  {
    role: "benefit",
    purpose:
      "Contrast isolated automated checks with a complete browser journey.",
    visualBrief:
      "Keep the clock-to-bug visual as the cost of missing integration failures.",
  ...
  },
  {
    role: "mechanism",
    purpose:
      "Explain why this is deeper than a generic browser automation demo.",
    visualBrief:
      "Keep the existing Claude plus Google Chrome logo composition.",
  ...
  },
  {
    ...
    visualBrief:
      "Keep the existing recording of installing and adding the Chrome extension.",
  ...
  },
  {
    role: "proof",
    purpose:
      "Show acceptance criteria becoming an actual end-to-end browser run.",
    visualBrief:
      "Keep the existing recording: open Claude, send the prompt, then show it navigating the website.",
  ...
  },
  {
    role: "cta",
    purpose:
      "Turn this into a recurring series about deeper Claude Code workflows.",
...
output.scenes = output.scenes.map((scene: any, index: number) => {
  ...
  const visuals = (scene.content.visuals ?? []).map(
    (visual: any, visualIndex: number) => {
      if (
        index === 3 &&
        visualIndex === 1 &&
        visual.visual?.type === "tool-logo"
      ) {
